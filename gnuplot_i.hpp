...
class Gnuplot
{
    ...
    // overloaded functions for plotting in JPEG file
    Gnuplot& saveToJpeg(const std::string &filename = "gnuplot_output");

    Gnuplot& saveToJpeg(const std::string &filename, int i);

    Gnuplot& saveToPng(const std::string &filename, int i);
    ...
    //inserted by KNR, overloaded function
    template<typename X, typename Y>
    Gnuplot& plot_xy(const X& x, const Y& y, const std::string &title = "", const std::string &append = "");
...
//
template<typename X, typename Y>
Gnuplot& Gnuplot::plot_xy(const X& x, const Y& y, const std::string &title, const std::string 				  &append)
{
    if (x.size() == 0 || y.size() == 0)
    {
        throw GnuplotException("std::vectors too small");
    ...
    plotfile_xy(name, 1, 2, title, append);
...
//------------------------------------------------------------------------------
// inserted by KNR
// saves a gnuplot session to a JPEG file
//
...
//
Gnuplot& Gnuplot::saveToPng(const std::string &filename, int i)
{
    cmd("set terminal png");

    std::ostringstream cmdstr;
    cmdstr << "set output \"" << filename << i << ".png\"";
    cmd(cmdstr.str());
...
//------------------------------------------------------------------------------
// Overloaded function to save images in sequence, inserted by KNR
// saves a gnuplot session to a JPEG file
//
Gnuplot& Gnuplot::saveToJpeg(const std::string &filename, int i)
{
    cmd("set terminal jpeg");

    std::ostringstream cmdstr;
    cmdstr << "set output \"" << filename << i << ".jpg\"";
    cmd(cmdstr.str());
...
/*************************************************************/
//inserted by KNR
//this is for getting specific point type, overloaded function
// Plots a 2d graph from a list of doubles (x y) saved in a file
//
Gnuplot& Gnuplot::plotfile_xy(const std::string &filename,
                              const unsigned int column_x,
			      ...
			      const std::string &append)
{
    ...
    cmdstr << "\"" << filename << "\" using " << column_x << ":" << column_y << append;
